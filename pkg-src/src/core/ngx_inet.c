...
char*
ngx_parse_host_port6(ngx_log_t *log, ngx_str_t *value, struct in6_addr *o_host, in_port_t *o_port, struct in6_addr dehost)
{
...
char*
ngx_parse_host_port(ngx_log_t *log, ngx_str_t *value, struct in_addr *o_host, in_port_t *o_port, in_addr_t dehost)
{
    if (value->len <= 0 || !o_host || !o_port)
        return NGX_CONF_ERROR ;
    ...
    if (port) { // with format of `host:port`
        ...
        if (tport != NGX_ERROR) {
            *o_port = htons((in_port_t) tport);
            ...
            ngx_log_debug0(NGX_LOG_ALERT, log, 0, "TCP redirect, config wrong port format.");
            return NGX_CONF_ERROR ;
        }
        ...
        if (thost != INADDR_NONE) {
            ...
            ngx_log_debug0(NGX_LOG_ALERT, log, 0, "TCP redirect, config wrong host format.");
            return NGX_CONF_ERROR ;
        }
    }
    else {
        ...
        if (tport != NGX_ERROR) { //with format of `port`
            *o_port = htons((in_port_t) tport);
            ...
            ngx_log_debug0(NGX_LOG_ALERT, log, 0, "TCP redirect, config without port[mandatory].");
            return NGX_CONF_ERROR ;
        }
    }
    return NGX_CONF_OK;
