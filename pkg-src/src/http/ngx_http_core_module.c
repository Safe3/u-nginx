...
static char *ngx_http_set_redirect(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
...
static char *
ngx_http_set_redirect(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_http_core_srv_conf_t *cscf = conf;
    ...
    if(cscf->redirect == NULL){
        ...
        if(cscf->redirect == NULL){
            ngx_conf_log_error(NGX_LOG_ALERT, cf, 0, "HTTP redirect, can not get array for redirect");
            return NGX_CONF_ERROR;
        }
    }
    ...
    if(t == NULL){
        ngx_conf_log_error(NGX_LOG_ALERT, cf, 0, "HTTP redirect, can not get elt for redirect");
        return NGX_CONF_ERROR;
    }
    ...
    ngx_conf_log_error(NGX_LOG_DEBUG, cf, 0, "HTTP set redirect %s -> %s",value[1].data,value[2].data);
