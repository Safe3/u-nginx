...
ngx_http_set_redirect(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if(ngx_strlchr(p,q,'[')!= NULL){ //with format of ipv6
        ...
        if(ngx_parse_host_port(cf->log, &value[1],&(t->host),&(t->port),htonl(INADDR_ANY))!=NGX_CONF_OK)
            return NGX_CONF_ERROR;
    }
    ...
    if(ngx_strlchr(p,q,'[')!= NULL){ //with format of ipv6
        ...
        if(ngx_parse_host_port(cf->log, &value[2],&(t->redirect_host),&(t->redirect_port),htonl(INADDR_LOOPBACK))!=NGX_CONF_OK)
            return NGX_CONF_ERROR;
    }
