    ...
    ngx_tcp_upstream_resolved_t *ur)
{
	...
	if(cscf->redirect){
        ...
        {
#if (NGX_HAVE_INET6)
            //TODO: not support ipv6 yet, just pass it
            case AF_INET6:
                break;
#endif
            case AF_INET: /* AF_INET */
                ...
                for(i=0;i< cscf->redirect->nelts;++i){
                    ngx_redirect_t *t = cscf->redirect->elts;
    ...
    if(ngx_strncmp(pc->name->data,"0.0.0.1:65535",pc->name->len) == 0){
        ngx_log_error(NGX_LOG_ALERT, s->connection->log, 0, "cannot est session for 0.0.0.1:65535");
        return NGX_DECLINED;
    }
